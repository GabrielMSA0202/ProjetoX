...
void CadastrarMembro(int pos){
    ...
    printf("|Sobrenome:");
    fgets(projetox[pos]->Sobrenome, sizeof (projetox[pos]->Sobrenome), stdin);
...
void ListarMembroUnico(int pos){
    for (int i = 0; i < 100; ++i) {
        if (projetox[i] != NULL && projetox[i]->Matricula == pos){
            ...
            printf("|NOME: %s  %s", projetox[i]->Nome, projetox[i]->Sobrenome);
            printf("|IDADE: %d\n", projetox[i]->Idade);
            printf("|E-MAIL: %s\n", projetox[i]->Email);
...
void AlterarDados(int pos){
    ...
    switch (opcao) {
        case 1:
            ...
            fgets(projetox[Verificador]->Sobrenome, sizeof (projetox[Verificador]->Sobrenome), stdin);
            printf("|NOME ALTERADO!\n");
            printf("|NOME: %s %s\n", projetox[Verificador]->Nome, projetox[Verificador]->Sobrenome);
            printf("---------------------------------------------------------\n");
            break;
        case 2:
